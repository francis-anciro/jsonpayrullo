...
<!--                    <span class="hidden text-xs font-bold uppercase tracking-widest text-zinc-500 md:block mr-4">Status & Actions</span>-->
                </div>
            </div>
            ...
            <div class="space-y-px overflow-hidden rounded-xl border border-zinc-800 bg-zinc-900/50 shadow-2xl">
                <?php foreach ($data['users'] as $user): ?>
                    <button type="button"
                            onclick="window.location.href='<?php echo URLROOT; ?>/users/edit/<?php echo $user->user_id; ?>'"
                            class="group flex w-full items-center justify-between border-b border-zinc-800/50 bg-zinc-900/30 px-6 py-4 text-left transition hover:bg-zinc-800/80 last:border-0 focus:outline-none focus:ring-2 focus:ring-inset focus:ring-indigo-500/50">
